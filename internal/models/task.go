...
type Task struct {
	...
	RequestParams    string               `json:"request_params" xorm:"TEXT"`                                 // HTTP POST 请求参数(JSON)
...
func (task *Task) UpdateBean(id int) (int64, error) {
	return Db.ID(id).
		Cols(`name,spec,protocol,command,timeout,multi,
			...
			notify_type,notify_receiver_id, dependency_task_id, dependency_status, tag,http_method, notify_keyword, request_params`).
		Update(task)
}
