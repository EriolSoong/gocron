  ...
  <div class="notif-page">
    ...
    <div class="notif-card">
      <el-form ref="formRef" :model="form" :rules="rules" label-position="top">
        <div class="section-title">🐦 飞书机器人配置</div>
        <el-form-item label="Webhook URL" prop="url"><el-input v-model="form.url" placeholder="https://open.feishu.cn/open-apis/bot/v2/hook/..." /></el-form-item>
        <el-form-item label="签名校验 (Secret)"><el-input v-model="form.secret" placeholder="可选，用于安全设置中的签名校验" /></el-form-item>
...
const groups = ref([])
const form = reactive({ url: '', secret: '', template: '' })
const rules = { url: [{ required: true, message: '请输入 Webhook URL', trigger: 'blur' }] }

onMounted(() => load())

function load() { notifService.feishu(data => { if (data) { Object.assign(form, data); groups.value = data.groups || [] } }) }
function submit() { formRef.value?.validate(v => { if (!v) return; notifService.updateFeishu({ ...form }, () => { ElMessage.success('保存成功'); load() }) }) }
function addGroup() { if (!newName.value) { ElMessage.warning('请输入名称'); return }; notifService.createFeishuGroup(newName.value, () => { showAdd.value = false; newName.value = ''; load() }) }
